...
if (sunvisibility > 0.01) {				
	...
	float sun = max(pow(max(1.0 - smoothCircleDist(1.0)/0.5,0.1),1.0)-0.1,0.0);
		
	vec3 lenscolorSunrise = vec3(2.52, 1.5, 0.8) * timeSunriseSunset;
	vec3 lenscolorNoon = vec3(1.5, 1.8, 2.0) * timeNoon;
				
	vec3 lenscolor = lenscolorSunrise + lenscolorNoon;
			
    float lens_strength = clamp(0.8 * lensBrightness - sun, 0.0, 1.0);
	     lenscolor *= lens_strength * 0.15;
			
	float sunray = max(pow(max(1.0 - yDistAxis(-0.7)/1.0,0.2),15.0)-0.7,0.0);
	float sunray1 = max(pow(max(1.0 - yDistAxis(10.0)/4.2,0.2),15.0)-0.7,0.0);	
    float sunray2 = max(pow(max(1.0 - yDistAxis(0.7)/1.0,0.2),15.0)-0.7,0.0);
    float sunray3 = max(pow(max(1.0 - yDistAxis(-4.0)/2.0,0.2),15.0)-0.7,0.0);	
	float sunrays = min(sunray+sunray1+sunray2+sunray3, 1.0);
				
	color += vec4(lenscolor * sunrays * visibility * sunvisibility * (1.0-rainStrength*1.0),1.0);
}
